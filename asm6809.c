...
static void _dump_opcodes( char *name, char *extn, word inst, op_arg *args ) {
	if( args ) {
		for( op_arg *ar = args; ar->mode != OP_NONE; ar++ ) {
			switch( ar->mode ) {
				case OP_INHERENT: {
					_displ_opcode( name, extn, "", inst + ar->mask );
					break;
				}
				case OP_IMM_BYTE: {
					_displ_opcode( name, extn, "#byte", inst + ar->mask );
					break;
				}
				case OP_IMM_WORD: {
					_displ_opcode( name, extn, "#word", inst + ar->mask );
					break;
				}
				case OP_DIRECT: {
					_displ_opcode( name, extn, ">value", inst + ar->mask );
					break;
				}
				case OP_EXTENDED: {
					_displ_opcode( name, extn, "<value", inst + ar->mask );
				...
				case OP_INDEXED: {
					_displ_opcode( name, extn, "indexed", inst + ar->mask );
					break;
				}
				case OP_SRELATIVE: {
					_displ_opcode( name, extn, "srel", inst + ar->mask );
					break;
				}
				case OP_LRELATIVE: {
					_displ_opcode( name, extn, "lrel", inst + ar->mask );
					break;
				}
				case OP_REG_PAIR: {
					_displ_opcode( name, extn, "r1,r2", inst + ar->mask );
					break;
				}
				case OP_REG_LIST: {
					_displ_opcode( name, extn, "r1..rn", inst + ar->mask );
					break;
				}
			}
		}
	...
	for( op_code *op = mc6809_op; op->name; op++ ) {
		if( op->exts ) {
			for( op_ext *ex = op->exts; ex->extn; ex++ ) {
				_dump_opcodes( op->name, ex->extn, op->base + ex->mask, op->args );
			}
		}
		else {
...
static a_token *organise_atomic( a_token *list, resync *toks, a_token **tree, bool log ) {
	...
	switch( list->tok ) {
		...
		case TOK_MINUS: {
			...
			*tree = list;
			list = organise_value( list->b, &sync, &( list->a ), log );
			break;
		}
		case TOK_NOT: {
			*tree = list;
...
static a_token *organise_reg_or_value( a_token *list, resync *toks, a_token **tree, arg_data *data, assemble_phase pass ) {
	...
	switch( list->tok ) {
		...
		default: {
			...
			data->value = (word)evaluate_tree( *tree, pass );
			data->op = OP_EXTENDED;
			break;
		}
	}
			...
			switch( list->b->tok ) {
				...
				case TOK_PLUSPLUS: {
					if( ind ) {
						...
						data->ea = ( list->b->tok == TOK_MINUS )? EA_POST_INC_ONE: EA_POST_INC_TWO;
					}
					data->i_reg = list->tok;
					*tree = list;
					list = list->b->b;
			...
			if(( head = organise_value( head, &sync, &ptr, true )) == NULL ) {
				...
				if( head->tok != TOK_EOS ) {
					...
					if( count < max ) {
						...
						//
						result[ count++ ] = (word)evaluate_tree( ptr, pass );
					}
					else {
...
static bool asm_align( int line, char *label, char *opcode, char *arg, assemble_phase pass ) {
	...
	if( analyse_value( arg, pass, 1, &val ) == 1 ) {
		...
		val = ( val << s )-1;
		this_address = ( this_address + val )&~val;
		if( label ) {
			if( !set_symbol( label, this_address, pass )) {
				if( pass == GATHER_PHASE ) {
